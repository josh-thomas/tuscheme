...
(* function [[ty]], checks type of expression, given $\itenvs$ ((prototype)) 357 *)
      | ty (AAT (a, i))        = 
              let val tau1 = ty a
                  val tau2 = ty i
                  val tau3 = ARRAYTY tau1
              in if eqType (tau2, INTTY) then
                        tau1
                    else
                      raise TypeError ("Index in at expression has type " ^
      ...
      | ty (APUT (a, i, e))    = 
              let val tau1 = ty a
                  val tau2 = ty i
                  val tau3 = ty e
                  val tau4 = ARRAYTY tau1
              in if eqType (tau2, INTTY) then
                        if eqType (tau1, tau3) then
                          tau3
                        else
                          raise TypeError ("The element in the expression has type " ^
                                           typeString tau3 ^ ", which should be " ^
                                           typeString tau1)
                  else
                        raise TypeError ("The index in the expression has type " ^
                                        typeString tau2 ^ ", which should be " ^
                                        typeString INTTY)
                end
      | ty (AMAKE (len, init)) = 
                let val tau1 = ty len
                    val tau2 = ty init
                in if eqType(tau1, INTTY) then tau2
                   else
                      raise TypeError ("The length has type " ^
                                       typeString tau1 ^ ", which should be " ^
                                       typeString INTTY)
                end
      | ty (ASIZE a)           = 
                let val tau1 = ty a
                    val tau2 = ARRAYTY tau1
                in INTTY
                end
                ;;(* let val tau1 = ty a
               ;; in if eqType (tau1, ARRAYTY) then INTTY
               ;;   else
              ;;        raise TypeError ("The element in the expression has type " ^
              ;;                          typeString tau1 ^ ", which should be " ^
              ;;                          typeString ARRAYTY)
