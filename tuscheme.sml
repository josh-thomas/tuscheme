    ...
    let fun ty (LITERAL V) = 
                      ...
                      (case v
                          of (NUM _)  => inttype
                           ...
                           | (PAIR (a, b)) => (case (a, b) of  
                                              (_, NIL) => listtype (literal a)
                                            | (_, _)   => 
                                              if eqType(listtype (literal a),
                                                                  literal b)
                                                then literal b
                                              else raise TypeError 
                                            "Expected lists of the same type")
                           | _ => raise TypeError "Not valid literal")
                in literal V end
          | ty (IFX (e1, e2, e3)) = 
                let val tau1 = ty e1
                    val tau2 = ty e2
                ...
                in if eqType(tau1, booltype) then
                      if eqType(tau2, tau3) then
                        ...
                        raise TypeError "e2 and e3 are not the same type"
                    else
                        raise TypeError "Expected a boolean in the first argument"
                end
          | ty (VAR x)     = find (x, t_env)
          | ty (APPLY (e, args)) = 
               (case ty e
                  of  (FUNTY (formaltypes, resulttype)) =>
                      if eqTypes(map ty args, formaltypes) then
                        resulttype
                      else
                         raise TypeError "Mismatch argmuent types"
                  | _ => raise TypeError "Apply expected a function")
          | ty (LETX (lk, bs, e)) =
               (case lk
                  of (LET) => 
                      let val n_env = bindList(map fst bs, map ty (map snd bs), t_env)
                          in typeof (e, k_env, n_env)
                          end
                   | (LETSTAR) => 
                        (case bs
                           of [] => ty e
                            | _  => ty (LETX (LET, List.take(bs, 1),
                                          LETX(LETSTAR, List.drop(bs, 1), e))))
                )
          | ty (LAMBDA (args, body)) = 
                  let val kindcheck = map (fn (x) => kindof (x, k_env)) (map snd args)                                                       
                    val n_env = bindList(map fst args, map snd args, t_env)
                  in FUNTY (map snd args, typeof(body, k_env, n_env))
                  end
          | ty (SET (x, e)) = 
               let val tau1 = find(x, t_env)
                   val tau2 = ty e
          ...
          | ty (WHILEX (e1, e2)) = 
               let val tau1 = ty e1
                ...
                in if eqType(tau1, booltype) then
                  ...
                  raise TypeError "WHILE expected a boolean expression"
                end
          | ty (BEGIN es) = 
               let fun b (e::es, lastval) = b (es, ty e)
                     | b (   [], lastval) = lastval
                in b (es, unittype)
                end
          ...
          | ty (LETRECX (bs, body)) =
                let val (tynames, values) = ListPair.unzip bs
                    ...
                    val types = map snd tynames
                    val exps   = map ty values
                    val kindcheck = map (fn (x) => kindof (x, k_env)) exps
                    val n_env = bindList (names, types, t_env)
                    val typecheck = map (fn (x) => typeof (x, k_env, n_env)) values
                in if List.all (fn (t) => (case t of 
                                                (LAMBDA (_, _)) => true
                                                | _             => false))
                                values
                   then typeof(body, k_env, n_env)
                   else raise TypeError ("Each expression must be a lambda")
                end

          | ty (TYLAMBDA (alphas, e)) = raise LeftAsExercise "Not Finished"
          | ty (TYAPPLY (e, taus)) =  raise LeftAsExercise "Not Finished"     
          
  in ty e 
  end 
  
fun typdef (d, k_env, t_env) =
    case d
      of
        VAL (x, e) => (bind (x, typeof (e, k_env, t_env), t_env), typeString (typeof(e, k_env, t_env)))
      | VALREC (x, tau, e) =>
            let val kindcheck = kindof (tau, k_env)
                val n_env     = bind   (x, tau, t_env)
                val typecheck = typeof (e, k_env, n_env)
            in if eqType(tau, typecheck) then
                  (n_env, typeString tau)
               else raise TypeError "x and e are not the same type"
            end
      | EXP e => typdef (VAL("it", e), k_env, t_env)
      | DEFINE (f, tau, (formals, body)) =>
          let val types = map snd formals
          in  (fst (typdef (VALREC(f, FUNTY(types, tau),
                              body), k_env, t_env)),
                              typeString tau)
          end
(* fun typdef (d, k_env, t_env) =
    let fun ty (VAL (x, e)) =
              let 
          | ty (VALREC (x, tau, e)) = raise LeftAsExercise "VAL"
          | ty (EXP (e))    = raise LeftAsExercise "EXP"
          | ty (DEFINE (f, tau, (vars, exp))) = raise LeftAsExercise "DEFINE"
    in ty d end *)
(* type declarations for consistency checking *)
